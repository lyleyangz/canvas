...
window.onload = function () {
    ...
    //
    var ParachuteRotate, ShockSounds, handsShock, GrassMove;
    ...
    function animateCanvas() {  //animateCanvas就是画每一帧动画的函数。
        ...
        clearCanvas();
        drawCanvas();
        (window).RAF(animateCanvas);  //动画更流畅  实现setInterval的效果
    }
     ...
     */
    function animateLoading(loadingImg, countImgDownload, loadingNumber, loadMax) {
     ...
     */
    function drawLoading(imgObj, processValue) {
    ...
    // 草丛位移
    CommonRotate.prototype.Grass = function (GrassMoveMaxDis, GrassCompany) {
    ...
    (window).addEventListener('touchend', e => {
        ...
        if (((musicPlay).currentTime == 0 && -currentScorll > 0)) {
            PointAtMusic = false;
            (musicPlay).play();
        }
    }, { passive: false });
